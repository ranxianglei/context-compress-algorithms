 ...
 *
 * retentionPct guard note: the divisor is `compressedTokens * 4`, so the
 * zero-guard belongs on `compressedTokens` (not `originalChars`). When
 * `compressedTokens === 0`, returns 0 → fails L1 retention rather than
 * producing `Infinity` (which would silently pass `< layer1MinRetentionPct`).
 */
