...
import { rougeRecallV1, DEFAULT_ROUGE_RECALL_V1_CONFIG } from "../src/quality-gate/rouge-recall-v1"
import type { QualityGateContext, BlockSnapshot } from "../src/quality-gate/types"

function makeBlock(overrides: Partial<BlockSnapshot> = {}): BlockSnapshot {
    return {
        blockId: 1,
...
function makeCtx(
    ...
    return {
        block: makeBlock({ compressedTokens }),
        summary,
...
test("Layer 1 retention guard uses compressedTokens for div-by-zero safety", () => {
    const summary = "x".repeat(250)
    const ctx = makeCtx(summary, "non-empty original text", 0)
    const result = rougeRecallV1.evaluate(ctx, DEFAULT_ROUGE_RECALL_V1_CONFIG)
    assert.equal(result.passed, false, "must fail when compressedTokens=0")
    assert.equal(result.layer, "L1-length")
    const metrics = Object.fromEntries(result.metrics.map((m) => [m.name, m.value]))
    assert.equal(metrics.retentionPct, 0, "retentionPct must be 0 when compressedTokens=0")
})
